...
sudo apt update && sudo DEBIAN_FRONTEND=noninteractive \
	...
	libxml-simple-perl libswitch-perl apt-utils libncurses5 curl python-is-python3 byobu rclone llvm -y || abort "Setup Failed!"
...
repofastsync() { time schedtool -B -e ionice -n 0 `which repo` sync -c --force-sync --optimized-fetch --no-tags --no-clone-bundle --retry-fetches=5 -j8 "$@"; }

aospainit() { time schedtool -B -e ionice -n 0 `which repo` init --depth=1 --no-repo-verify -u https://github.com/AOSPA/manifest -b uvite -g default,-mips,-darwin,-notdefault
 "$@"; }
